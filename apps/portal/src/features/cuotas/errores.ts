/**
 * Traduce los errores de negocio de `declarar_cuotas` a mensajes claros en español.
 * La RPC ya lanza en español, pero con ids crudos; aquí damos algo accionable para el admin.
 */
export function mapearErrorDeclaracion(error: unknown): string {
  const e = error as { code?: string; message?: string } | null
  const code = e?.code
  const msg = (e?.message ?? '').toLowerCase()

  if (code === '42501' || msg.includes('no autorizado')) {
    return 'No tienes permiso para declarar cuotas de esta empresa.'
  }
  ...
  if (msg.includes('inactivo')) {
    return 'Hay un colaborador inactivo en la declaración. Recarga la lista de colaboradores.'
  }
  ...
  if (msg.includes('fin de semana')) {
    return 'Solo se pueden declarar comidas de lunes a viernes.'
  }
  if (msg.includes('ya pasó') || msg.includes('ya paso') || msg.includes('pasada')) {
    return 'No se pueden declarar comidas en fechas pasadas.'
  }
  return 'No se pudo guardar la declaración. Intenta de nuevo.'
}
