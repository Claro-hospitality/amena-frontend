...
/**
 * Abre una ventana aislada con el pase del invitado (logo + QR + nombre + día) y lanza
 * "Guardar como PDF". `imagenQR` es el dataURL del canvas del QR (alta resolución).
 */
export function descargarPasePDF({
  nombre,
  apellido,
  fecha,
  empresaNombre,
  ...
  const nombrePase = escaparHtml(`${nombre}${apellido ? ` ${apellido}` : ''}`)
  const empresa = empresaNombre ? escaparHtml(empresaNombre) : ''
  const dia = escaparHtml(etiquetaDia(deISO(fecha)))
